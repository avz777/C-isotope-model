...
co2fr={}
co2to={}
#Read CO2atm data for zones from flie Zone,CO2min,CO2max
input2 = open("Zones_CO2.txt", "r")
...
for line in text1:
    ...
    co2fr[zone]=int(items2[1])
    co2to[zone]=int(items2[2])
...
# & cintrol reality of the temperature values (+30 C limit for the sea water)
    tc=15.7-4.36*(dop-dow)+0.12*(dop-dow)*(dop-dow)
    if tc>30: tc=30
    t.append(tc)
...
# ddic=dccarb+1.0 (Romanek et al., 1992; Hartke et al., 2021)

# dco=ddic-(-0.095t + 10.72) (equation is based on data from Yoshioka, 1997)
...
# dccon(calculated)= dcph-0.113*dcph-1.916=0.887*dcph-1.916 (Caut et al., 2009) for invertebrates
#
# dccon(calculated)= 0.787*dcph-2.848 (Caut et al., 2009) for vertebrates
...
#possible individual variations are assumed as about +-1.2 promille
